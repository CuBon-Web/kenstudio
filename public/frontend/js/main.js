(function ($) {
  ...
  $(document).ready(function () {
    // Image Comparison Slider (init after images load so container gets height)
    function initComparisonBox($box) {
      if ($box.hasClass("twentytwenty-container")) {
        return;
      }
      ...
      function tryInit() {
        ...
        $box.find(".twentytwenty-handle").each(function () {
      ...
      $imgs.each(function () {
        if (this.complete) {
          ...
          $(this).one("load error", tryInit);
        }
      });
    }

    function initBeforeAfterComparisons() {
      var $boxes = $(".antra-image-comparison").not(".twentytwenty-container");
      if (!$boxes.length) {
        return;
      }

      if ("IntersectionObserver" in window) {
        var compObserver = new IntersectionObserver(
          function (entries) {
            entries.forEach(function (entry) {
              if (entry.isIntersecting) {
                initComparisonBox($(entry.target));
        ...
        $boxes.each(function () {
          compObserver.observe(this);
        });
      } else {
        $boxes.each(function () {
          initComparisonBox($(this));
        });
      }
    }

    // Init twentytwenty cho tất cả .antra-image-comparison bên trong một container
    function initTwentyTwentyIn($container) {
      $container.find(".antra-image-comparison").each(function () {
        var $box = $(this);
        if ($box.hasClass("twentytwenty-container")) return;
        if ($box.find("img").length < 2) return;
    ...
    // Chuyên cho project carousel: lưu swiper để window.load có thể gọi
    function initProjectComparisons(swiper) {
      window._projectSwiper = swiper;
    }

    // Init twentytwenty sau khi toàn bộ ảnh trên trang đã load xong
    $(window).on("load", function () {
      // Project carousel
      if (window._projectSwiper) {
        initTwentyTwentyIn($(window._projectSwiper.el));
      }
      // Project grid (section project-item-5, ngoài Swiper)
      initTwentyTwentyIn($("body"));
    });
    ...
    // Data Background (eager for hero/header; lazy for below-fold)
    function applyDataBackground($el) {
    ...
    $("[data-background].bg-eager").each(function () {
      applyDataBackground($(this));
    });

    var $lazyBgs = $("[data-background]").not(".bg-eager");
    ...
    var swiperProject = new Swiper(".project-carousel", {
      ...
      on: {
        init: function () {
          initProjectComparisons(this);
        },
        slideChangeTransitionEnd: function () {
          initTwentyTwentyIn($(this.el));
        },
      },
    });
