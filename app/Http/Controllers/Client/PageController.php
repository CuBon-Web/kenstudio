...
class PageController extends Controller
{
    ...
    public function postcontact(Request $request){
        ...
        if ($redirectUrl && strpos($redirectUrl, url('/')) === 0) {
            return redirect($redirectUrl)->with('success', 'Gửi yêu cầu thành công. Chúng tôi sẽ liên hệ sớm.');
        }
        if ($data) {
    ...
    {
        $data['detail'] = Project::where('slug', $slug)->where('status', 1)->with('cateProject')->firstOrFail();
