    ...
    <script>
        (function($) {
            ...
            function getDetailPayload() {
                ...
                var hasVariant = Number($actions.data("has-variant")) === 1;
                var variant = hasVariant ? String($("#detail-selected-variant").val() || "") : "";
                var price = hasVariant ? Number($actions.data("variant-price") || 0) : 0;
            ...
            $(document).on("click", "#detail-buy-now-btn", function(e) {
                ...
                postDetailAddToCart(payload, function(err) {
                    ...
                    showDetailNotify("Đã thêm sản phẩm, chuyển tới thanh toán", "success");
                    if (checkoutUrl) {
            ...
            $(document).on("click", ".detail-variant-option", function() {
                var $actions = $("#detail-product-actions");
                if (Number($actions.data("has-variant")) !== 1) {
                    return;
                }

                var variantMap = $("#detail-variant-wrap").data("price-map") || {};
                var variantKey = String($("#detail-selected-variant").val() || "");
                var nextPrice = variantMap[variantKey];
                if (typeof nextPrice === "undefined") {
                    nextPrice = Number($actions.data("variant-price") || 0);
                }
                $actions.data("variant-price", Number(nextPrice || 0));
            });
        })(jQuery);
    </script>
@endsection
@section('content')
        ...
        <div class="container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ languageName($product->cate->name) }}</li>
                    <li class="breadcrumb-item active" aria-current="page">
                        {{ $product->seo_title ? $product->seo_title : $product->name }}</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="shop-details-top-section mt-60 mb-60">
        <div class="container-xl container-fluid-lg container">
            <div class="row gy-5">
                <div class="col-lg-6">
                    <div class="shop-details-img">
                        <div class="tab-content" id="v-pills-tabContent">
                            @forelse ($img as $index => $image)
                            ...
                            @empty
                                <div class="tab-pane fade show active" id="v-pills-img1" role="tabpanel"
                                    ...
                                    <div class="shop-details-tab-img product-img--main" data-scale="1.4" data-image="">
                        ...
                        @if (count($img) > 0)
                            <div class="detail-thumb-nav-wrap">
                                <button type="button" class="detail-thumb-arrow detail-thumb-prev"
                                    aria-label="Xem ảnh trước">
                                    <i class="bi bi-chevron-up"></i>
                                </button>
                                <div class="detail-thumb-nav-viewport">
                                    <div class="nav nav-pills detail-thumb-nav" id="v-pills-tab" role="tablist"
                                        aria-orientation="vertical">
                                        @foreach ($img as $index => $image)
                                            <button class="nav-link {{ $index == 0 ? 'active' : '' }}"
                                                id="v-pills-img{{ $index + 1 }}-tab" data-bs-toggle="pill"
                                                data-bs-target="#v-pills-img{{ $index + 1 }}" type="button"
                                                role="tab" aria-controls="v-pills-img{{ $index + 1 }}"
                                ...
                                <button type="button" class="detail-thumb-arrow detail-thumb-next"
                ...
                <div class="col-lg-6">
                    <div class="shop-details-content">
                        @php
                            ...
                            if ((int) $product->status_variant === 1) {
                                $variantRows = \App\models\VariantSkuValue::where('product_id', $product->id)
                                    ->orderBy('id', 'ASC')
                                    ->get(['version', 'price', 'sku']);

                                foreach ($variantRows as $row) {
                                    $variantPriceMap[$row->version] = (float) $row->price;
                                    $variantSkuMap[$row->version] = (string) ($row->sku ?? '');
                                }

                                $rawVariant = json_decode((string) $product->variant, true);
                                if (is_array($rawVariant)) {
                                    foreach ($rawVariant as $opt) {
                                        $labels = collect($opt['option_values'] ?? [])
                                            ->pluck('label')
                                            ->filter()
                                            ->values()
                                            ->all();
                                        if (!empty($opt['display_name']) && count($labels) > 0) {
                                            $variantOptions[] = [
                                                'name' => $opt['display_name'],
                                                'values' => $labels,
                                            ];
                                            $selectedVariantValues[] = (string) $labels[0];
                                        }
                                    }
                                }

                                $selectedVariantKey = implode(' - ', $selectedVariantValues);
                                if (!empty($selectedVariantKey) && isset($variantPriceMap[$selectedVariantKey])) {
                                    $selectedVariantPrice = (float) $variantPriceMap[$selectedVariantKey];
                                } elseif ($variantRows->count() > 0) {
                                    $selectedVariantPrice = (float) $variantRows->min('price');
                                }

                                if (!empty($selectedVariantKey) && !empty($variantSkuMap[$selectedVariantKey])) {
                                    $selectedVariantSku = $variantSkuMap[$selectedVariantKey];
                                }
                            }
                        @endphp
                        <h1>{{ $product->name }}</h1>
                        ...
                        <div class="price-area">
                            @if (!is_null($selectedVariantPrice))
                                <p class="price" id="detail-product-price">{{ number_format($selectedVariantPrice) }}₫
                        ...
                        <div class="quantity-color-area">
                            <div class="quantity-color">
                                <h6 class="widget-title">Số lượng</h6>
                            ...
                            @if (count($variantOptions) > 0)
                                <div id="detail-variant-wrap" data-price-map='@json($variantPriceMap)'
                                    data-sku-map='@json($variantSkuMap)' data-base-sku="{{ $product->sku }}"
                                    data-original-price="{{ number_format($originalPrice) }}₫"
                                    data-fallback-price="{{ !is_null($selectedVariantPrice) ? $selectedVariantPrice : $salePrice }}">
                                    @foreach ($variantOptions as $groupIndex => $option)
                                        <div class="quantity-color detail-variant-group"
                                            data-group-index="{{ $groupIndex }}">
                                            <h6 class="widget-title">{{ $option['name'] }}: <span
                                                    class="detail-selected-value">{{ $option['values'][0] }}</span></h6>
                                            <ul class="color-list detail-variant-list">
                                                @foreach ($option['values'] as $valueIndex => $value)
                                                    <li class="select-wrap detail-variant-option {{ $valueIndex === 0 ? 'selected' : '' }}"
                                                        data-group-index="{{ $groupIndex }}"
                                                        data-value="{{ $value }}">
                                                        <span>{{ $value }}</span>
                                                    </li>
                                    ...
                                    @endforeach
                                    <input type="hidden" id="detail-selected-variant" value="{{ $selectedVariantKey }}">
                                </div>
                            @endif
                        </div>
                        <div class="shop-details-btn" id="detail-product-actions" data-product-id="{{ $product->id }}"
                            data-add-cart-url="{{ route('add.to.cart') }}" data-checkout-url="{{ route('checkout') }}"
                            data-has-variant="{{ (int) $product->status_variant === 1 ? 1 : 0 }}"
                            data-variant-price="{{ !is_null($selectedVariantPrice) ? $selectedVariantPrice : 0 }}">
                            <a href="#" class="primary-btn1 hover-btn3" id="detail-add-cart-btn">Thêm giỏ
                                hàng</a>
                        ...
                        <div class="product-info">
                            <ul class="product-info-list">
                                <li> <span>SKU:</span> <span
                                        id="detail-product-sku">{{ $selectedVariantSku ?: $product->sku }}</span></li>
                                <li> <span>Thương hiệu:</span> <a href="{{ route('home') }}">Covyshrimp</a></li>
                                <li> <span>Danh mục:</span> <a
                                        href="">{{ languageName($product->cate->name) }}</a></li>
                            </ul>
                        </div>
                        <ul class="product-shipping-delivers">
                            <li class="product-shipping">
                                ...
                                Miễn phí vận chuyển toàn quốc trên tất cả đơn hàng trên 300.000đ
                            </li>
                            <li class="product-delivers">
                                ...
                                <p>Giao hàng trong: 3-7 ngày làm việc <a href="#">Vận chuyển &amp; Trả hàng</a></p>
                            </li>
                        </ul>
                    </div>
                </div>
                        ...
                        <nav>
                            <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                <button class="nav-link active" id="nav-description-tab" data-bs-toggle="tab"
                                    ...
                                    aria-controls="nav-description" aria-selected="true">Chi tiết sản phẩm</button>
                                <button class="nav-link" id="nav-add-info-tab" data-bs-toggle="tab"
                                    data-bs-target="#nav-add-info" type="button" role="tab"
                                    aria-controls="nav-add-info" aria-selected="false">Thông số bổ sung</button>
                            </div>
                        </nav>
                    </div>
                            ...
                            <div class="tab-pane fade" id="nav-add-info" role="tabpanel"
                                ...
                                <div class="addithonal-information">
                                    ...
                                    <table class="table total-table2">
                                        <tbody>
                                            @forelse($technicalSpecs as $spec)
                                                <tr>
